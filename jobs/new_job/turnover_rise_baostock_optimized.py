...
def login_baostock() -> bool:
    ...
    try:
        ...
        if lg.error_code == '0':
            ...
            logger.error(f"✗ Baostock登录失败 - 错误码: {lg.error_code}, 错误信息: {lg.error_msg}")
            return False
...
def get_stock_data(code: str, start_date: str, end_date: str, adjust: str = "3") -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if rs.error_code != '0':
            logger.error(f"✗ [{code}] 查询失败 - 错误码: {rs.error_code}, 错误信息: {rs.error_msg}")
            return None
    ...
    try:
        ...
        for index, row in data.iterrows():
            try:
                ...
                logger.debug(f"  [{symbol}] 数据已存在或主键冲突: {row['date']} - {str(e)}")
                fail_count += 1
            except Exception as e:
                logger.error(f"✗ [{symbol}] 插入数据失败: {row['date']} - {str(e)}")
                fail_count += 1
