...
CONFIG = {
    ...
    'MAX_RUNTIME_MINUTES': 240,  # 任务最长运行时间，超过则强制收尾退出
}
...
class StockDataProcessor:
        ...
        try:
            if not self.bs_logged_in:
                ...
                if result.error_code == '0':
                    ...
                    raise Exception(f"Baostock login failed: {result.error_msg}")
        except Exception as e:
            logger.error(f"✗ Baostock 登录异常: {e}", exc_info=True)
            raise
    ...
    def fetch_minute_data(self, code: str, name: str, retry_count: int = 0) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if rs.error_code != '0':
                logger.warning(f"  ⚠ {code} [{name}] 无数据: {rs.error_msg}")
                return None
